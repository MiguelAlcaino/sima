...
class Presupuestos extends CI_Controller 
{
  function __construct()
  {
	...
	{	
		...
		$codigo        = $this->presupuestos->get_max_cod();
		
		$data['clientes']  = $this->clientes->get_all_order_name();
		$data['data']  = $this->presupuestos->get_all();
		$data['cod']   = $this->codigo($codigo->cod);
		$this->template->display('admin/presupuestos/list',$data);
	...
	function editar($id){
		...
		$this->template->display('admin/presupuestos/editar',$data);
		
	}
	
	function agregar()
	{	
			$this->presupuestos->add();	
	...
	function imprimir($id=0)
	{
		...
		$data    = $this->presupuestos->get_by_id($id);
    ...
    $html2pdf->Output('Presupuesto '.$data[0]['numero'].'.pdf', 'D');
}
